...
var historyList = [];
var currentAdmin = '';
var SK = 'nomor-pkb-ksp-cusmg-v1';
var HK = 'nomor-pkb-history-ksp-cusmg-v1';
var AS = 'ksp-cusmg-admin-session-v1';
var AP = '1';
...
function saveHistory() {
  try { localStorage.setItem(HK, JSON.stringify(historyList)); } catch (e) {}
}
function saveAdminSession() {
  try { sessionStorage.setItem(AS, JSON.stringify({ isAdmin: isAdmin, name: currentAdmin || '' })); } catch (e) {}
}
...
function load() {
  ...
  try {
    var h = localStorage.getItem(HK);
    historyList = h ? JSON.parse(h) : [];
    if (!Array.isArray(historyList)) historyList = [];
  } catch (e) {
    historyList = [];
  }
  try {
    var a = sessionStorage.getItem(AS);
    var s = a ? JSON.parse(a) : null;
    isAdmin = !!(s && s.isAdmin);
    currentAdmin = s && s.name ? s.name : '';
  } catch (e) {
    isAdmin = false;
    currentAdmin = '';
  }
...
function loginAdmin() {
  ...
  if (!adminName) {
    toast('Nama admin wajib diisi', true);
    return;
  }
  if (document.getElementById('adminPass').value !== AP) {
    toast('Password admin tidak sesuai', true);
    return;
  }
...
function addHistoryLog(action, beforeItem, afterItem) {
  historyList.unshift({
    ...
    before: summarizePkb(beforeItem || {}),
    after: afterItem ? summarizePkb(afterItem) : 'Data dihapus'
  });
